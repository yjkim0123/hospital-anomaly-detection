...
% Fix for ieeeaccess.cls xfigwd bug
\makeatletter
\ifdefined\xfigwd\else\xdef\xfigwd{0pt}\fi
\makeatother
...
\address[1]{Department of Software, Ajou University, Suwon 16499, South Korea (e-mail: yjkim0123@ajou.ac.kr; ORCID: 0000-0003-4234-4883)}
...
The COVID-19 pandemic catalyzed renewed interest in hospital financial vulnerability. Cutler~\cite{cutler2020impact} documented the pandemic's impact on U.S. healthcare finances, while Khullar et al.~\cite{khullar2020covid} specifically examined the financial health of U.S. hospitals during the pandemic. In Korea, Lee~\cite{lee2024covid} provided an observational study of the pandemic's differential impact on hospital financial performance according to hospital characteristics. In a companion study using the same Korean hospital dataset, Kim and Sun~\cite{kim2024financial} applied panel data analysis combined with supervised machine learning (Random Forest, Gradient Boosting) to predict financial distress, identifying material cost ratio and year effects as key predictors. The present paper complements that supervised approach by providing an unsupervised alternative that does not require binary deficit labels, making it applicable to surveillance scenarios where such labels are unavailable or unreliable. The supervised and unsupervised approaches address fundamentally different analytical questions: supervised methods require binary labels and predict known outcomes, while unsupervised methods identify structurally unusual patterns without predefined categories, potentially revealing risks invisible to label-dependent approaches.
...
\multicolumn{4}{l}{\footnotesize Note: With Bonferroni correction for 11 tests ($\alpha = 0.05/11 \approx 0.0045$),} \\
\multicolumn{4}{l}{\footnotesize all six significant features remain significant.}
\end{tabular}
\end{table}
...
Of the 342 deficit observations (41.4\% of the sample), only 27 overlap with the 57 consensus anomalies. Thus, only 7.9\% of deficit hospitals are flagged as consensus anomalies, and 47.4\% of consensus anomalies are in deficit. The chi-squared test yields $\chi^2 = 0.67$ ($p = 0.414$); we cannot reject the null hypothesis of independence between anomaly status and deficit status.
...
\subsection{Comparison with Altman Z''-Score Distress}
\label{subsec:zscore}

To further contextualize our unsupervised approach, we compare consensus anomalies against a traditional credit scoring baseline: Altman's Z''-score~\cite{altman2002revisiting}, a variant designed for non-manufacturing and private firms. Using hospital financial data, we compute $Z'' = 6.56 X_1 + 3.26 X_2 + 6.72 X_3 + 1.05 X_4$, where $X_1$ = working capital/total assets, $X_2$ = retained earnings/total assets, $X_3$ = EBIT/total assets, and $X_4$ = book value of equity/total liabilities. For hospitals, we substitute medical operating income for EBIT and accumulated surplus/deficit for retained earnings.

Applying the standard distress threshold ($Z'' < 1.1$) classifies 58.2\% of observations as financially distressed---substantially higher than both the consensus anomaly rate (6.9\%) and the deficit rate (41.4\%). Strikingly, the overlap between Z-score distress and consensus anomaly is minimal: only 32 observations (3.9\% of total) are flagged by both methods, yielding a Jaccard similarity of just 0.063. The chi-squared test shows no significant association ($\chi^2 = 0.03$, $p = 0.856$, $\phi = 0.006$).

This independence reveals a critical insight: consensus anomalies capture \textit{different} hospitals than traditional credit scoring. While Z-score distress flags 58.2\% of observations based primarily on capital structure ratios, consensus anomalies identify a smaller subset (6.9\%) characterized by unusual \textit{operational} cost patterns. Among hospitals flagged \textit{only} by the Z-score (not anomaly), the deficit rate is 51.4\%. Among hospitals flagged \textit{only} as anomalies (not Z-score distress), the deficit rate is 40.0\%. Hospitals flagged by \textit{both} methods show a deficit rate of 53.1\%, while those flagged by \textit{neither} have the lowest rate at 26.2\%.

The complementary nature of these approaches suggests that combining traditional credit scoring with unsupervised anomaly detection may yield more comprehensive financial surveillance than either method alone---Z-scores capture capital structure stress while consensus anomalies identify operational pattern deviations.
...
\textbf{Public hospital confound.} We acknowledge that public hospitals constitute 54.4\% of consensus anomalies despite representing only 25.5\% of the sample. This overrepresentation partially reflects genuine structural differences in public hospital finances rather than operational dysfunction. However, the consensus mechanism captures multidimensional financial patterns beyond ownership alone, as evidenced by the contribution of cost structure ratios and subsidy dependence to anomaly scores.

\textbf{Circular validation.} A methodological limitation of unsupervised anomaly detection validation is its inherently circular nature: algorithms identify statistically unusual observations, and subsequent analysis confirms their unusual characteristics. The key validation in our framework is not that anomalies have unusual values---which is tautological---but rather that (1) they do NOT simply replicate deficit labels (47.4\% overlap, $p=0.414$), demonstrating that the approach captures distinct financial patterns, and (2) temporal trends reveal increasing financial heterogeneity. Future work should pursue external validation against concrete outcomes such as hospital closures, regulatory interventions, or credit rating downgrades.

\textbf{Consensus threshold.} The consensus threshold of three out of five algorithms represents a majority-voting rule, a natural choice that balances sensitivity and specificity. A lower threshold (2/5) would increase sensitivity at the cost of more false positives, while a higher threshold (4/5) would yield fewer but more robust anomalies. In deployment, this threshold should be treated as a tunable parameter calibrated to the regulator's tolerance for false positives versus missed detections.

\textbf{Contamination parameter.} Setting the contamination parameter to 0.1 introduces a semi-supervised assumption about the expected anomaly rate. While this value was chosen based on domain expectations, the true anomaly rate is unknown. The sensitivity analysis in Section~\ref{subsec:sensitivity} partially addresses this concern, but future work should explore contamination-free approaches or data-driven threshold selection.

\textbf{Survivorship bias.} The balanced panel design, while eliminating compositional bias, introduces survivorship bias by excluding hospitals that closed during 2019--2023. These excluded institutions may represent the most severe cases of financial distress, potentially causing our analysis to underestimate both the prevalence and severity of financial anomalies.

\textbf{Simple baselines.} We acknowledge that simple rule-based approaches---such as flagging hospitals exceeding predefined ratio thresholds---could serve as practical baselines. The advantage of the unsupervised approach is that it does not require predefined thresholds and can detect multivariate patterns invisible to univariate rules. Nevertheless, future work should formally compare detection performance against rule-based and supervised alternatives.
...
Critically, the limited overlap between unsupervised anomalies and accounting deficits (47.4\%) demonstrates that anomaly detection captures structurally unusual financial patterns beyond simple profitability, potentially offering a complementary surveillance tool for healthcare regulators. Combined with SHAP-based explainability, the framework provides both detection capability and actionable interpretability.
...
Generative AI tools were used for language editing and formatting assistance during manuscript preparation.
All scientific content, data analysis, interpretation, and conclusions are solely the work of the authors, who take full responsibility for the accuracy and integrity of this paper.

\section*{Conflict of Interest Statement}
The authors declare that there are no conflicts of interest.

\section*{Acknowledgment}
The authors thank the Korea Health Industry Development Institute (KHIDI) for providing publicly available hospital financial data.

\bibliographystyle{IEEEtran}
\bibliography{refs}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{kim_photo.jpg}}]{Yongjun Kim}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{sun_photo.jpg}}]{Eun Jung Sun}
